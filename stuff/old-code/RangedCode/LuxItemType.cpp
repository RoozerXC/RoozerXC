...
cLuxItemType_Ranged::cLuxItemType_Ranged() : iLuxItemType("Ranged", eLuxItemType_Ranged)
{
...
bool cLuxItemType_Ranged::BeforeAddItem(cLuxInventory_Item *apItem)
{
	ProgLog(eLuxProgressLogLevel_Medium, "Picked up ranged");

	gpBase->mpHintHandler->Add("PickRanged", kTranslate("Hints", "PickRanged"), 0);
...
void cLuxItemType_Ranged::OnUse(cLuxInventory_Item *apItem, int alSlotIndex)
{
	cLuxPlayerRanged *pRanged = gpBase->mpPlayer->GetHelperRanged();
	pRanged->SetActive(!pRanged->IsActive(), true);
}

//-----------------------------------------------------------------------

tWString cLuxItemType_Ranged::GetDisplayedNameAdd(cLuxInventory_Item *apItem)
{
	cLuxPlayerRanged *pRanged = gpBase->mpPlayer->GetHelperRanged();
	tWString sStr = pRanged->IsActive() ? kTranslate("Inventory", "RangedOn") : kTranslate("Inventory", "RangedOff");
	return _W(" (") +sStr + _W(")");
}
