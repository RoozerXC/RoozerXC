...
void cLuxPlayerInsanityCollapse::Start()
{
	...
	mpPlayer->GetHelperRanged()->SetActive(false,false, false);
...
cLuxPlayerRanged::cLuxPlayerRanged(cLuxPlayer *apPlayer) : iLuxPlayerHelper(apPlayer, "LuxPlayerRanged")
{
	Reset();
}

cLuxPlayerRanged::~cLuxPlayerRanged()
{
...
void cLuxPlayerRanged::OnStart()
{

}

void cLuxPlayerRanged::Reset()
{
	mbDisabled = false;
	mbActive = false;
}
...
void cLuxPlayerRanged::Update(float afTimeStep)
{
	if(mbActive ==false)
	{
		return;
...
void cLuxPlayerRanged::OnMapEnter(cLuxMap *apMap)
{
	
}

void cLuxPlayerRanged::OnMapLeave(cLuxMap *apMap)
{
...
void cLuxPlayerRanged::CreateWorldEntities(cLuxMap *apMap)
{
	cWorld *pWorld = apMap->GetWorld();
...
void cLuxPlayerRanged::DestroyWorldEntities(cLuxMap *apMap)
{
...
void cLuxPlayerRanged::SetActive(bool abX, bool abUseEffects, bool abCheckForItem, bool abCheckIfAllowed)
{
	if(mbActive == abX) return;

	/////////////////
	// Check so allowed
	if(abCheckIfAllowed && mpPlayer->GetCurrentStateData()->AllowRanged()==false)
	{
		return;
	}

	/////////////////
	// Check so player has the ranged item
    if(abCheckForItem && gpBase->mpInventory->HasItemOfType(eLuxItemType_Ranged)==false)
	{
		gpBase->mpHintHandler->Add("RangedNoItem", kTranslate("Hints", "RangedNoItem"), 0);
		return;
	}
	...
	if(mbActive)
	{
		if(abUseEffects) gpBase->mpHelpFuncs->PlayGuiSoundData("", eSoundEntryType_Gui);
	}
	else
	{
		if(abUseEffects) gpBase->mpHelpFuncs->PlayGuiSoundData("", eSoundEntryType_Gui);
	...
	// Hand
	if(mbActive)
		mpPlayer->GetHands()->SetActiveHandObject("ranged");
	else
		mpPlayer->GetHands()->SetActiveHandObject("");
}
...
void cLuxPlayerRanged::SetDisabled(bool abX)
{
	mbDisabled = abX;
